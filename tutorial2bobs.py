...
# File: tutorial2bobs.py
# Auth: Gary E. Deschaines
# Date: 23 May 2015
# Prog: Double pendulum system modeled with PyODE, animated with Pygame
# Desc: Models numerical solution for double bob pendulum system dynamics 
...
# joints to hinge, added integration of the differential
# equations of motion using Runge-Kutta 4th order method
# (RK4), and utilized matplotlib to generate plots of data
# collected from the ODE simulation and RK4 integration)
#
...
# Set following to False to model physical double pendulum system
USE_LINEARIZED = True   # True: linearized ideal double pendulum (ref [1])
USE_IDEALIZED  = False  # True: idealized double pendulum (ref [3]) 
                        # False: physical double pendulum (ref [4])
...
Phi0   = Theta0_2  # initial angle (P) of 2nd rod attached to 1st rod
if USE_LINEARIZED:
    USE_IDEALIZED = False
    Phi0 = Phi0 - Theta0
...
if USE_LINEARIZED: ang = Theta0 + Phi0
else             : ang = Phi0

SPHERE2_POSITION = vecAdd(SPHERE1_POSITION,rodAngleToXYZ(ang))
...
#
# The non-linearized differential equations of motion of an idealized 
# double pendulum are those given for w1' and w2' in the "Numerical 
# Solution" section of the web page provided by reference [3].
# 
# The non-linearized differential equations of motion of a physical
# double pendulum are those given for xdot(3) and xdot(4) on page 26
# of reference [4].
#
# References:
#
...
def dotS(n,S):
    ...
    if USE_LINEARIZED:
    ...
    elif USE_IDEALIZED: # from eqs for w1 prime and w2 prime of ref [3]
        cosS1    = cos(S[1])
        sinS1    = sin(S[1])
        S1mS3    = S[1] - S[3]
        cosS1mS3 = cos(S1mS3)
        ...
        twoS1    = 2.0*S[1]
        twoS3    = 2.0*S[3]
        denom    = 2.0*m1 + m2*(1.0 - cos(twoS1-twoS3))
        denom1   = L1*denom
        denom2   = L2*denom
        dS[1] = S[2]
        dS[2] = ( -(2.0*m1 + m2)*g*sinS1 - m2*g*sin(S[1]-twoS3) \
                    - 2.0*m2*sinS1mS3*(L2*S4sq + L1*S2sq*cosS1mS3) ) / denom1
        dS[3] = S[4]
        dS[4] = ( 2.0*sinS1mS3 * \
                    (m1pm2*L1*S2sq + m1pm2*g*cosS1 + m2*L2*S4sq*cosS1mS3) ) / denom2
    else: # from eqs for xdot(3) and xdot(4) on pg 26 in App. F of ref [4]
        sinS1      = sin(S[1])
        sinS3      = sin(S[3])
        S1mS3      = S[1] - S[3]
        cosS1mS3   = cos(S1mS3)
        sinS1mS3   = sin(S1mS3)
        cosS1mS3sq = cosS1mS3**2
        S2sq       = S[2]**2
        S4sq       = S[4]**2
        denom      = -m2sq*h1sq*d2sq*cosS1mS3sq +\
                      I1*I2 + I1*m2*d2sq + m2*h1sq*I2 + m2sq*h1sq*d2sq
                    
        dS[1] = S[2]
        dS[2] = -( m2*h1*d2*S4sq*sinS1mS3*I2 +\
                   m2sq*h1*d2**3*S4sq*sinS1mS3 +\
                   m1*g*d1*sinS1*I2 + m1*g*d1*sinS1*m2*d2sq +\
                   m2*g*h1*sinS1*I2 + m2sq*g*h1*sinS1*d2sq +\
                   m2sq*h1sq*d2sq*cosS1mS3*S2sq*sinS1mS3 -\
                   m2sq*h1*d2sq*cosS1mS3*g*sinS3) / denom            
        dS[3] = S[4]
        dS[4] = ( h1sq*cosS1mS3*m2*d2*S4sq*sinS1mS3 +\
                  h1*cosS1mS3*m1*g*d1*sinS1 + h1sq*cosS1mS3*m2*g*sinS1 +\
                  h1*S2sq*sinS1mS3*I1 + h1**3*S2sq*sinS1mS3*m2 -\
                  g*sinS3*I1 - g*sinS3*m2*h1sq)*m2*d2 / denom
...
if pygame.font:
    ...
    if USE_LINEARIZED:
        ...
        text = font.render("PyODE Tutorial 2 - Dble Pendulum (idealized)", 1, TEXT_COLOR)
    else:
        text = font.render("PyODE Tutorial 2 - Dble Pendulum (physical)", 1, TEXT_COLOR)
...
if __name__ == "__main__":
    ...
    while loopFlag and i < nSamples:
        ...
        if USE_LINEARIZED:
            ...
            m2Pos = vecAdd(rodAngleToXYZ(S[3]),m1Pos)
        ...
        if SAVE_ANIM:
            ...
            fpth = "./anim/tutorial2bobs_" + istr + ".png"
        ...
        # Calculate linear accelerations of ODE bodies by applying Articulated
        # -Body forward dynamics algorithm with joint feedback forces.
        ...
        if PRINT_DATA:
            print("j1:  angle (deg) = %8.3f  theta (deg) = %8.3f  omega (rad/sec) = %8.3f" % \
                  (j1angle*DPR, j1theta*DPR, j1omega) )
        ...
        if PRINT_DATA:
            print("j2:  angle (deg) = %8.3f  theta (deg) = %8.3f  omega (rad/sec) = %8.3f" % \
                  (j2angle*DPR, j2theta*DPR, j2omega) )
        ...
        if USE_LINEARIZED: angVel = vecMulS(JOINT2_AXIS,(S[2]+S[4]))
        else             : angVel = vecMulS(JOINT2_AXIS,S[4])
        ...
        if PLOT_DATA:
            ...
            if USE_LINEARIZED: B2AVrk4[i] = (S[2]+S[4])*DPR
            else             : B2AVrk4[i] = S[4]*DPR
        # ... translational kinetic energy
        #     refer to eqs. (2.5) and (2.6) on pg. 4 of reference [4]
        ...
        if USE_LINEARIZED:
            ...
            m2KErot = 0.5*BODY_IZZc*S[4]**2
        '''
        # ... potential (relative to origin at joint 1 anchor)
        #     refer to eq. (2.13) on pg. 5 of reference [4]
        b1PE = BODY_MASS*g*b1Pos[1]
        b2PE = BODY_MASS*g*b2Pos[1]
        m1PE = BODY_MASS*g*m1Pos[1]
        m2PE = BODY_MASS*g*m2Pos[1] 
        '''
        # ... potential energy (relative to Theta0 and Phi0 equal to 0)
        #     refer to eq. (2.13) on pg. 5 of reference [4]
        b1PE = -2*BODY_MASS*g*ROD_LENGTH*cos(j1theta)
        b2PE = -BODY_MASS*g*ROD_LENGTH*cos(j2theta)
        m1PE = -2*BODY_MASS*g*ROD_LENGTH*cos(S[1])
        if USE_LINEARIZED:
            m2PE = -BODY_MASS*g*ROD_LENGTH*cos(S[1]+S[3])
        else:
            m2PE = -BODY_MASS*g*ROD_LENGTH*cos(S[3])
        ...
        '''
        # ... rotational kinetic energy
        b1KErot = 0.5*(BODY_IZZc + 2*BODY_MASS*ROD_LENGTH**2)*j1omega**2 +\
                  BODY_MASS*ROD_LENGTH**2*j1omega*(j1omega+j2omega)*cos(j1theta-j2theta)
        b2KErot = 0.5*(BODY_IZZc + BODY_MASS*ROD_LENGTH**2)*(j1omega+j2omega)**2
        m1KErot = 0.5*(BODY_IZZc + 2*BODY_MASS*ROD_LENGTH**2)*S[2]**2 +\
                  BODY_MASS*ROD_LENGTH**2*S[2]*S[4]*cos(S[1]-S[3])
        m2KErot = 0.5*(BODY_IZZc + BODY_MASS*ROD_LENGTH**2)*S[4]**2
        # ... potential energy (relative to Theta0 and Phi0 equal to 0)
        b1PE = -2*BODY_MASS*g*ROD_LENGTH*cos(j1theta)
        b2PE = -BODY_MASS*g*ROD_LENGTH*cos(j2theta)
        m1PE = -2*BODY_MASS*g*ROD_LENGTH*cos(S[1])
        m2PE = -BODY_MASS*g*ROD_LENGTH*cos(S[3])
        # ... total system energy (relative to minimum potential energy)
        TEode = b1KErot + b2KErot + b1PE + b2PE - MIN_PE
        TErk4 = m1KErot + m2KErot + m1PE + m2PE - MIN_PE
        '''
        if PRINT_DATA:
